...
try:  # robust import for script/module modes
...
class ResidualScaler:
    ...
    def __post_init__(self):
        safe_scales = np.asarray(self.scales, dtype=np.float64).reshape(-1)
        object.__setattr__(self, "scales", safe_scales)
        comp_map = {
            name: float(scale)
            for name, scale in zip(self.component_names, safe_scales)
        }
        object.__setattr__(self, "_component_map", comp_map)

    def normalize_tensors(self, tensors: Sequence):
        normalized = []
...
def build_residual_scaler(
    ...
    *,
    ...
    m_q = df["m_Q"].to_numpy(dtype=np.float64, copy=False)
    m_l = df["m_L"].to_numpy(dtype=np.float64, copy=False)
    g_vals = df["g"].to_numpy(dtype=np.float64, copy=False)
    total_mass = m_q + m_l
    force_scale = _reduce_statistic(total_mass * g_vals, normalized_mode, eps)

    rho_vectors = df[RHO_COLUMNS].to_numpy(dtype=np.float64, copy=False)
    rho_norm = np.linalg.norm(rho_vectors, axis=1)
    # Lever arm: 优先使用 l_length（若存在），否则回退至 |rho|
    if "l_length" in df.columns:
        lever_candidates = df["l_length"].to_numpy(dtype=np.float64, copy=False)
        lever_candidates = np.where(np.isfinite(lever_candidates), lever_candidates, rho_norm)
    else:
    ...
    parallel_scale = _reduce_statistic(rho_norm, normalized_mode, eps)
    ...
    scale_r3 = _apply_scale_override(parallel_scale, overrides.get("r3"), eps)
...
def resolve_sample_plot_specs(raw: str | None) -> List[SamplingPlotSpec]:
    if not raw:
        return list(DEFAULT_SAMPLE_PLOT_SPECS)
...
class ConstantManager:
    ...
    def auxiliary(self, inputs: np.ndarray) -> np.ndarray:
        ...
        if missing:
            ...
            print(
                f"[force_estimation][ConstantManager] fallback samples used "
                f"{missing} (total {self._fallback})."
            )
...
def build_force_estimation_residual(
    ...
    """Create the physics residual function with the provided feature normalizer."""
    ...
    def residual(x, y, aux_vars):
        ...
        f_Q = y[:, 0:3]
        f_L = y[:, 3:6]
        rho = features[:, IDX_RHO]

        acc_q = features[:, IDX_AQ]  # ẍ_Q
        acc_l = features[:, IDX_AL]  # ẍ_L
        thrust = features[:, IDX_THRUST : IDX_THRUST + 1]
        m_q = const_mgr.slice_tensor(consts, "m_Q")
        m_l = const_mgr.slice_tensor(consts, "m_L")
        g_scalar = const_mgr.slice_tensor(consts, "g")

        ez_world = features[:, IDX_EZ]
...
def compute_residual_components_numpy(
    ...
    if predictions.ndim != 2 or predictions.shape[1] != OUTPUT_DIM:
        raise ValueError(
            f"predictions 必须为形状 (N, {OUTPUT_DIM}) 的数组，得到 {predictions.shape}"
        )
    ...
    rho = features[:, IDX_RHO]
    ez_world = features[:, IDX_EZ]
    thrust = features[:, IDX_THRUST].reshape(-1, 1)

    m_q = df["m_Q"].to_numpy(dtype=np.float64, copy=False).reshape(-1, 1)
    m_l = df["m_L"].to_numpy(dtype=np.float64, copy=False).reshape(-1, 1)
    g_scalar = df["g"].to_numpy(dtype=np.float64, copy=False).reshape(-1, 1)
