...
def load_training_data(num):
    ...
    # training domain 训练领域: X × Y = [1, 8] × [−2, 2] and T = [0, 7]
    ...
    # choose number of training points 选择训练点数: num =7000
...
# Parameters to be identified 待识别的参数
C1 = dde.Variable(0.0)
C2 = dde.Variable(0.0)

# Define Navier Stokes Equations (Time-dependent PDEs) 定义纳维-斯托克斯方程（时间相关偏微分方程）
...
# Training datasets and Loss 训练数据集和损失函数
...
# callbacks for storing results 用于存储结果的回调函数
fnamevar = "variables.dat"
variable = dde.callbacks.VariableValue([C1, C2], period=100, filename=fnamevar)

# Compile, train and save model 编译、训练并保存模型
...
dde.saveplot(loss_history, train_state, issave=True, isplot=True)
# model.save(save_path = "./NS_inverse_model/model") 模型保存
f = model.predict(ob_xyt, operator=Navier_Stokes_Equation)
print("Mean residual:", np.mean(np.absolute(f)))

# Plot Variables 情节变量:
# reopen saved data using callbacks in fnamevar 使用fnamevar中的回调重新打开保存的数据
lines = open(fnamevar, "r").readlines()
# read output data in fnamevar 读取fnamevar中的输出数据
Chat = np.array(
    [
        np.fromstring(
...
# Plot the velocity distribution of the flow field:绘制流场的速度分布图：
for t in range(0, 8):
