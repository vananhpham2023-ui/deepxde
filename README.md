...
## Project Extension: estimation (Force Estimation PINN)

This repository includes a project-specific package under `estimation/` for a
physics‑informed neural network that estimates interaction forces in a
quadrotor–payload system. It is built on top of DeepXDE and organized as a
clean CLI pipeline for data → geometry → physics → training.

- Entry: `estimation/cli.py` (script wrapper: `estimation/force_estimation.py`)
- Dataset (default): `estimation/datasets/force_estimation_train.csv`
- Monitoring: `metrics_logger.py` (CSV logging) and `gradient_monitor.py`
- Residual scaling and weighting: `physics.py`, `weight_scheduler.py`

Quick demo (PyTorch backend):
```
...
Important CLI flags (Stage 3 features):
- `--residual-norm-mode {off,median,mean}` and `--residual-scale-r{1,2,3}`
- `--loss-weight-mode {manual,inverse_residual}`, `--loss-weight-r{1,2,3}`, `--bc-loss-weight`
- `--adaptive-weights` with `--adaptive-weight-{period,alpha,min,max}`
- `--metrics-csv` for stepwise CSV logging; `--grad-monitor` to log grad L2

Backward compatibility: example shims under `examples/pinn_inverse/` re-export
APIs from `estimation/` so existing example paths continue to run.
